<template>
    <div class="bg-slate-200 w-full h-[100vh] relative">
        <div class="absolute top-[20%] left-0 right-0 w-full max-w-sm py-4 px-6 bg-slate-100 mx-auto shadow-md rounded-md">
            <h1 class="text-4xl mt-2 mb-4 text-slate-700">Sign In<span class="text-base font-medium"> (Manager)</span></h1>
                ...
                <div class="flex flex-wrap">
                    ...
                    <div class="w-full">
                        <div class="flex flex-row items-center gap-x-3">
                            ...
                            <RouterLink class="text-base font-medium text-blue-400" :to="{name: 'manager_forgot_password',query:{next: undefined}}">Forgot Password?</RouterLink>
                        </div>
                        <p class="my-2.5 text-slate-600 text-base font-medium">Don't have an account? 
                            <RouterLink class="text-blue-400" :to="{name: 'manager_signup'}">Sign Up</RouterLink> now
                        </p>
                    </div>
                </div>
           </form>
    ...
    export default {
        ...
        methods: {
            async submit(){
                ...
                if(validate){
                    this.$store.dispatch('user_signin',{'data':data,'role':'manager','next':next})
                }
            }
        }
    }
