    ...
    <div class="relative w-full bg-slate-200 min-h-[90vh] top-[10vh]">
        <div class="flex flex-col mx-6">
            <div class="w-full py-3 px-3 bg-slate-100 my-10 shadow-md">
                <h1 class="text-center sm:text-left text-4xl text-slate-700">Request Section</h1>
            </div>
        </div>
        <div class="flex md:flex-row flex-col md:justify-between items-center mx-6">
            <div class="my-2 flex flex-row shadow-sm rounded-md overflow-hidden">
                <input class="sm:w-[30vw] md:w-[25vw] lg:w-[20vw] max-w-md px-3 h-10 bg-slate-100 outline-none text-slate-500 font-medium placeholder:text-slate-400 placeholder:font-medium" placeholder="Find Your Request Here" type="search">
                <button>
            ...
            <div class="flex flex-row items-center space-x-1 cursor-pointer">
                ...
                <RouterLink :to="{name: 'manager_create_request'}" class="text-xl text-slate-600 font-medium">Make Requests</RouterLink>
            </div>
        </div>
                    ...
                    <table class="w-[75vw] text-sm font-light text-center">
                        <thead class="border-b-2 border-neutral-500 text-base">
                            <tr class="text-slate-600 uppercase">
                                <th scope="col" class="px-6 py-3">Date of Request</th>
                                ...
                                <th scope="col" class="px-6 py-3">Delete Request</th>
                            </tr>
                        </thead>
                        <tbody v-if="requests&&requests.length">
                            <tr v-for="request in requests_latest" :key="request.id" class="text-center border-b border-slate-400 font-medium text-base">
                                <td class="whitespace-nowrap px-6 py-4">{{ RequestedDate(request.request_date) }}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{ `${request.request_type} Category` }}</td>
                                ...
                                <td class="whitespace-nowrap px-6 py-4">{{ request.status }}</td>
                                <td class="whitespace-nowrap px-6 py-4 flex flex-row text-sm justify-center space-x-2">
                                    <span @click="onDeleteRequest($event,request.id)" class="p-2 bg-red-500 rounded-md shadow-md hover:text-white cursor-pointer">Delete</span>
                                </td>
                            </tr>
                        </tbody>
...
import moment from 'moment'
    ...
    export default {
        ...
        created(){
            this.$store.dispatch('get_manager_requests')
        },
        methods: {
            onDeleteRequest(e,id){
                this.$store.dispatch('delete_manager_request',id)
            },
            RequestedDate(date){
