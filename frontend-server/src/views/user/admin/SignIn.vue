<template>
    <div class="bg-slate-200 w-full h-[100vh] relative">
        <div class="absolute top-[20%] left-0 right-0 w-full max-w-sm py-4 px-6 bg-slate-100 mx-auto shadow-md rounded-md">
            <h1 class="text-4xl mt-2 mb-4 text-slate-700">Sign In<span class="text-base font-medium"> (Admin)</span></h1>
    ...
    export default {
        ...
        methods: {
            async submit(){
                ...
                if(validate){
                    this.$store.dispatch('user_signin',{'data':data,'role':'admin','next':next})
                }
            }
        }
    }
