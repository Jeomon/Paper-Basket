<template>
    ...
    <div class="absolute top-[10vh] right-0 -z-10 w-[60%] sm:w-[70%] lg:w-[80%] min-h-[90vh] bg-slate-200">
        <div v-if="productBasedData.length" class="bg-slate-100 w-full flex flex-row flex-wrap items-center p-2.5 gap-x-4">
            <p v-for="(ribbon,index) of ribbonData" :key="index" @click="ribbonHandler(index)" :ref="`ribbon_${index}`" class="bg-slate-200 px-2 py-1.5 my-1 text-center hover:bg-white cursor-pointer shadow-sm rounded-3xl font-medium text-base text-slate-600">{{ ribbon }}</p>
        </div>
        <div class="container mx-auto px-5 sm:px-10 my-10 font-semibold">
            ...
            <div v-else class="mx-6 mt-[20%]">
                <p class="text-center text-2xl text-slate-500">No Products Available</p>
            </div>
        </div>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed: {
            ...
            ribbonData(){
                let ribbon=['Latest Products','Discount','Rating','Price','In Stock']
                return ribbon
            },
            filterBasedData(){
            ...
            ribbonBasedData(){
                ...
                switch(this.ribbonBtnTracker){
                    case 0: {
                        ribbonData=this.productBasedData
                        break
                    }
                    case 1: {
                    ...
                    case 4:{
                        ribbonData=this.productBasedData.filter(product=>product.inventory>0)
                    }
                    default: {
